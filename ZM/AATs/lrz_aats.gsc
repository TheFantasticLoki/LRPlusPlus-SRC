...
init()
{
	//level flag_wait( "initial_blackscreen_passed" );
	if(getdvar("CUSTOM_MAP") != 4  || level.script != "zm_tomb")
	{
...
actor_damage_override( inflictor, attacker, damage, flags, meansofdeath, weapon, vpoint, vdir, shitloc, psoffsettime, boneindex ) 
{
	...
	if(isdefined( attacker.weaponname ))
	{
		...
		{
			switch(self.vip_status)/*getdvar("Sys_LRZ_P_" + self.guid + "_VIP_Status")*/
			{
				case "Developer":
					aat_cooldown_time = randomintrange(4, 10); //cooldown 10 - 15 seconds
	        		aat_activation = randomintrange(1,5); //bullet that actives aat 1 - 10 
					break;
				default:
...
cluster()
{
	switch(self.vip_status)
	{
		case "Developer":
			...
			{
				amount = randomIntRange(7, level.round_number) * 4;
			}
			break;
		case "Contributor":
			if(level.round_number < 10)
			...
			{
				amount = randomIntRange(7, level.round_number) * 3;
			}
			break;
		case "Friend":
			if(level.round_number < 10)
		...
		case "VIP++":
			...
			{
				amount = randomIntRange(7, level.round_number) * 2;
			}
			break;
		case "VIP+":
			if(level.round_number < 10)
	...
	for(i = 0; i < amount; i++)
	{
		self MagicGrenadeType( "frag_grenade_zm", self.origin + (random_x, random_y, 10), (random_x, random_y, 0), 0.75 );
		wait .1;
	}
}
