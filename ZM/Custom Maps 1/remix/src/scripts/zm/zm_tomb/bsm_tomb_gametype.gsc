...
onspawnplayer( predictedspawn ) //modified function
{
	...
	if ( !isDefined( spawnpoint ) )
	{
		...
		spawnpoints = [];
		if ( isDefined( level.CUSTOM_MAP ) && level.CUSTOM_MAP == "trenches" )
		{
...
get_player_spawns_for_gametype() //modified function
{
	...
	custom_spawns = [];
	if ( isDefined( level.CUSTOM_MAP ) && level.CUSTOM_MAP == "crazyplace" )
	{
		for ( i = 0; i < level.crazyplaceSpawnpoints.size; i++ )
		{
	...
	else if ( isDefined( level.CUSTOM_MAP ) && level.CUSTOM_MAP == "trenches" )
	{
		for ( i = 0; i < level.trenchesSpawnpoints.size; i++ )
		{
