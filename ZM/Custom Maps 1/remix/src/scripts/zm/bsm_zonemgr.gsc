...
manage_zones( initial_zone ) //checked changed to match cerberus output
{
	...
	if ( isDefined( level.CUSTOM_MAP ) && level.CUSTOM_MAP == "redroom" )
	{
		initial_zone = [];
		initial_zone[ 0 ] = "zone_orange_level3b";
	}
	if ( isDefined( level.CUSTOM_MAP ) && level.CUSTOM_MAP == "rooftop" )
	{
		initial_zone = [];
	...
	if ( isDefined( level.CUSTOM_MAP ) && level.CUSTOM_MAP == "showers" )
	{
		initial_zone = [];
	...
	else if ( isDefined( level.CUSTOM_MAP ) && level.CUSTOM_MAP == "tank" )
	{
		initial_zone = [];
...
create_spawner_list( zkeys ) //modified function
{
	...
	for ( z = 0; z < zkeys.size; z++ )
	{
		...
		if ( zone.is_enabled && zone.is_active && zone.is_spawning_allowed )
		{
			...
			{
				if(level.script == "zm_transit" && level.CUSTOM_MAP != "0")
				{
				...
				else if (level.script == "zm_prison" && level.CUSTOM_MAP != "0")
				{
