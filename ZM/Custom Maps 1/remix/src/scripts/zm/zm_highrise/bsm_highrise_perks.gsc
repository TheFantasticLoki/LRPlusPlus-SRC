...
perk_machine_spawn_init() //modified function
{
	...
	{
		if(is_true(level.disableBSMMagic) || level.CUSTOM_MAP == "building1top" || level.CUSTOM_MAP == "redroom" )
		{
		...
		if ( isdefined( structs[ i ].script_string ) )
		{
			...
			while ( k < tokens.size )
			{
				...
				{
					if(is_true(level.CUSTOM_MAP == "redroom") && structs[i].script_noteworthy == "specialty_weapupgrade")
					{
						structs[i] Delete();
					}
					else
