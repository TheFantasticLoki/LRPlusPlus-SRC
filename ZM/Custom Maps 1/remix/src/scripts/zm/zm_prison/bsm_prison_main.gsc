...
alcatraz_afterlife_doors() //checked changed to match cerberus output
{
	...
	if ( isDefined( self.script_noteworthy ) && self.script_noteworthy == "afterlife_door" )
	{
		...
		if ( isDefined ( level.CUSTOM_MAP ) && level.CUSTOM_MAP == "cellblock" || isDefined ( level.CUSTOM_MAP ) && level.CUSTOM_MAP == "rooftop" )
		{
...
{
	if(isdefined(level.CUSTOM_MAP) && level.CUSTOM_MAP != "0" && !GetDvarIntDefault("useBossZombies", 1))
	{
