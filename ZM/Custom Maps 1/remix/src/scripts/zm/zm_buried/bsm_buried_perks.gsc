...
perk_machine_spawn_init() //modified function
{
	...
	while ( i < structs.size )
	{
		...
		if(is_true(level.CUSTOM_MAP == "maze"))
		{
			structs[i].origin = (0,0,-10000);
		}
	...
	if ( isdefined(level.CUSTOM_MAP) && level.CUSTOM_MAP == "maze" && !is_true(level.disableBSMMagic) )
	{
	...
	for ( i = 0; i < pos.size; i++ )
	{
		...
		if ( IsDefined( perk ) && IsDefined( pos[ i ].model ) )
		{
			...
			if(level.script == "zm_buried" && level.CUSTOM_MAP == "maze")
			{
