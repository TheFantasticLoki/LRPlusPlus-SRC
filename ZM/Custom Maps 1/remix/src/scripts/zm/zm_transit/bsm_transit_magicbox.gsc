...
init_magicbox() //modified function
{
	...
	if ( is_classic() )
	{
		...
		if (isDefined(level.CUSTOM_MAP) && level.CUSTOM_MAP == "tunnel" )
		{
			level.chests = [];
		...
		else if (isDefined(level.CUSTOM_MAP) && level.CUSTOM_MAP == "cornfield" )
		{
			level.chests = [];
			start_chest = spawnstruct();
		...
		else if (isDefined(level.CUSTOM_MAP) && level.CUSTOM_MAP == "house" )
		{
			level.chests = [];
			start_chest = spawnstruct();
		...
		else if (isDefined(level.CUSTOM_MAP) && level.CUSTOM_MAP == "power" )
		{
			level.chests = [];
			start_chest = spawnstruct();
		...
		else if (isDefined(level.CUSTOM_MAP) && level.CUSTOM_MAP == "diner" )
		{
			level.chests = [];
			start_chest = spawnstruct();
	...
	else
	{
		...
		else if(getDvar("CUSTOM_MAP") == "busdepot" )
		{
...
get_chest_pieces() //modified function
{
	...
	if ( isDefined( level.CUSTOM_MAP ) && level.CUSTOM_MAP == "tunnel" && self.script_noteworthy == "start_chest" )
	{
		self.chest_box.origin = ( -11090, -349, 195 );
		self.chest_box.angles = ( 0, -100, 0 );
	}
	if ( isDefined( level.CUSTOM_MAP ) && level.CUSTOM_MAP == "tunnel" && self.script_noteworthy == "farm_chest" )
	{
		self.chest_box.origin = ( -11772, -2501, 229 );
		self.chest_box.angles = ( 0, 0, 0 );
	}
	if ( isDefined( level.CUSTOM_MAP ) && level.CUSTOM_MAP == "diner" && self.script_noteworthy == "depot_chest" )
	{
		self.chest_box.origin = ( -5708, -7968, 229 );
		self.chest_box.angles = ( 0, 1, 0 );
	}
	if ( isDefined( level.CUSTOM_MAP ) && level.CUSTOM_MAP == "power" && self.script_noteworthy == "depot_chest" )
	{
		self.chest_box.origin = ( 10806, 8518, -407 );
		self.chest_box.angles = ( 0, 180, 0 );
	}
	if ( isDefined( level.CUSTOM_MAP ) && level.CUSTOM_MAP == "cornfield" && self.script_noteworthy == "start_chest" )
	{
		self.chest_box.origin = ( 13566, -541, -188 );
		self.chest_box.angles = ( 0, -90, 0 );
	}
	if ( isDefined( level.CUSTOM_MAP ) && level.CUSTOM_MAP == "cornfield" && self.script_noteworthy == "depot_chest" )
	{
		self.chest_box.origin = ( 7458, -464, -196 );
		self.chest_box.angles = ( 0, -90, 0 );
	}
	if ( isDefined( level.CUSTOM_MAP ) && level.CUSTOM_MAP == "cornfield" && self.script_noteworthy == "farm_chest" )
	{
		self.chest_box.origin = ( 10158, 49, -220 );
		self.chest_box.angles = ( 0, -185, 0 );
	}
	if ( isDefined( level.CUSTOM_MAP ) && level.CUSTOM_MAP == "house" && self.script_noteworthy == "start_chest" )
	{
	...
	else if ( isDefined( level.CUSTOM_MAP ) && level.CUSTOM_MAP == "tunnel" && self.script_noteworthy == "farm_chest" )
	{
		collision = spawn( "script_model", self.chest_box.origin );
		collision.angles = self.chest_box.angles;
		collision setmodel( "collision_clip_32x32x128" );
