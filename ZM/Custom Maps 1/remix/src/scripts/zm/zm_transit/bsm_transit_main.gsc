...
override_zombie_count() //custom function
{
	...
	if( level.CUSTOM_MAP == "house" || level.CUSTOM_MAP == "cornfield" )
	{
	...
	for ( ;; )
	{
		...
		else if ( level.CUSTOM_MAP == "cornfield" )
		{
...
{
	if ( level.CUSTOM_MAP != "cornfield" && level.CUSTOM_MAP != "house" )
	{
		return;
	}
	while ( 1 )
