...
main()
{
	if(GetDvar("CUSTOM_MAP") == "")
		SetDvar("CUSTOM_MAP", "0");
	level.CUSTOM_MAP = GetDvar("CUSTOM_MAP");
	level.disableBSMMagic = 0;
	if(level.CUSTOM_MAP == "0")
		return;
...
init()
{
	...
	if ( isDefined ( level.CUSTOM_MAP ) && level.CUSTOM_MAP != "0" || getDvar("CUSTOM_MAP") == "farm")
	{
...
init_custom_map()
{
	...
	if(level.script == "zm_highrise" && is_true(level.CUSTOM_MAP != "0") || level.script == "zm_buried" && is_true(level.CUSTOM_MAP != "0"))
		thread power_setup();
	wait 5;
	thread map_fixes();
}
...
perkHud()
{
	if(level.script != "zm_prison" && level.script != "zm_highrise")
		return;
	if(isdefined(level.CUSTOM_MAP) && level.CUSTOM_MAP == "0")
		return;
	self endon("disconnect");
	level endon("end_game");
...
map_fixes()
{
	if ( level.script == "zm_prison" && isDefined( level.CUSTOM_MAP ) && level.CUSTOM_MAP == "docks" )
	{
		level notify( "cable_puzzle_gate_afterlife" );
	}
	else if ( level.script == "zm_prison" && isDefined( level.CUSTOM_MAP ) && level.CUSTOM_MAP == "cellblock" )
	{
...
init_buildables()
{
	if ( isDefined( level.CUSTOM_MAP ) && level.CUSTOM_MAP == "tunnel" || isDefined( level.CUSTOM_MAP ) && level.CUSTOM_MAP == "diner" || isDefined( level.CUSTOM_MAP ) && level.CUSTOM_MAP == "power" || isDefined( level.CUSTOM_MAP ) && level.CUSTOM_MAP == "cornfield" || isDefined( level.CUSTOM_MAP ) && level.CUSTOM_MAP == "house")
	{
		wait 1;
	...
	if ( level.script == "zm_prison" && isDefined( level.CUSTOM_MAP ) && level.CUSTOM_MAP != "0" )
	{
		wait 2;
		buildcraftable( "quest_key1" );
		buildcraftable( "alcatraz_shield_zm" );
		if ( isDefined( level.CUSTOM_MAP ) && level.CUSTOM_MAP == "cellblock" )
		{
			buildcraftable( "packasplat" );
			buildcraftable( "plane" );
		}
		else if ( isDefined ( level.CUSTOM_MAP ) && level.CUSTOM_MAP == "docks" )
		{
			buildcraftable( "plane" );
		}
		else if ( isDefined ( level.CUSTOM_MAP ) && level.CUSTOM_MAP == "rooftop" )
		{
	...
	if ( level.script == "zm_highrise" && isDefined( level.CUSTOM_MAP ) && level.CUSTOM_MAP != "0" )
	{
		...
		removebuildable( "sq_common" );
		if(is_true(level.CUSTOM_MAP == "building1top"))
		{
			level.zombie_include_weapons[ "slipgun_zm" ] = 1;
			level.zombie_weapons[ "slipgun_zm" ].is_in_box = 1;
		}
	}
	if ( level.script == "zm_tomb" && isdefined( level.CUSTOM_MAP ) && level.CUSTOM_MAP != "0" )
	{
...
getPerks()
{
	...
	if ( isDefined( level.zombiemode_using_chugabud_perk ) && level.zombiemode_using_chugabud_perk && level.CUSTOM_MAP != "building1top" && level.CUSTOM_MAP != "redroom")
	{
